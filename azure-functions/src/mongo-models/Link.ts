  ...
  constructor(
    ...
    if (!link || link.length < 3 || link.length > 2048) {
      throw CustomError.invalid(
        "Link is invalid. Must be between 3 and 2048 characters (inclusive)."
      );
    }

    if (!mapping || mapping.length < 3 || mapping.length > 64) {
      throw CustomError.invalid(
        "Mapping is invalid. Must be between 3 and 64 characters (inclusive)."
      );
    }
    ...
    for (const forbiddenMapping of FORBIDDEN_MAPPINGS) {
      if (mapping === forbiddenMapping) {
        throw CustomError.invalid(
          `Mapping is invalid. Cannot be '${forbiddenMapping}'.`
        );
      }
    }
    ...
    if (!/^[a-zA-Z0-9-]+$/.test(mapping)) {
      throw CustomError.invalid(
        "Mapping is invalid. Must only contain characters, numbers and dashes."
      );
    }
